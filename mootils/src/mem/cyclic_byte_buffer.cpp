...
namespace mem {
    ...
    bool CyclicByteBuffer::empty() const noexcept {
        return m_buf.empty();
    }

    std::span<std::byte> CyclicByteBuffer::span() noexcept {
        return m_buf.span();
    }

    std::span<const std::byte> CyclicByteBuffer::span() const noexcept {
        return m_buf.span();
    }
    ...
    void CyclicByteBuffer::clear() noexcept {
        reset();
        m_buf.clear();
    }
}